...
class Testmfapy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        target_fragments = {
            ...
            'AKGmsms': {
                'atommap': 'AKG_1:2:3:4:5+AKG_1+AKG_2:3:4:5',
                'order': 9,
                'type': 'msms',
                'use': 'no',
                'formula': 'no data'
        ...
        self.model = mfapy.metabolicmodel.MetabolicModel(
            reactions, reversible, metabolites, target_fragments)
    ...
    def test_load_states(self):
        ...
        self.assertEqual(expected, actual)
        flux_list = [
            state_dic["reaction"][id]['value']
            for id in self.model.reaction_ids
        ]
        expected = [
            100.0, 100.0, 50.0, 50.0, 50.0, 125.0, 75.0, 50.0, 0.0, 50.0, 1.0,
            0.0, 100.0, 1.0, 1.0, 1.0, 1.0, 0.0
        ]
        actual = flux_list
        self.assertEqual(expected, actual)
    ...
    def test_matrix_inv(self):
        ...
        self.model.update()
        tmp_r = [
            state_dic[group][id]["value"]
            for (group, id) in self.model.vector["ids"]
        ]
        ind_r = [
            state_dic[group][id]["value"]
            for (group, id) in self.model.vector["independent_flux"]
        ]
        import numpy

        Rm_initial = list(self.model.vector["Rm_initial"])
        Rm_initial[self.model.numbers["independent_start"]:self.model.
                   numbers["independent_end"]] = ind_r
        flux_list = [
            state_dic["reaction"][id]['value']
            for id in self.model.reaction_ids
        ]
        expected = [
            100.0, 100.0, 50.0, 50.0, 50.0, 125.0, 75.0, 50.0, 0.0, 50.0,
            100.0, 0.0, 100.0, 0.5, 1.0, 1.0, 1.0, 0.5, 10.0, 10.0, 10.0, 10.0,
            10.0, 100.0, 100.0, 100.0, 100.0, 100.0, 50.0, 50.0
        ]
    ...
    def test_calmdv(self):
        ...
        glu, glut = self.model.func["calmdv"](flux_list, ['Glue'],
                                              cs.generate_dict())
        expected = [
            0.34635416666666663, 0.26953124999999994, 0.27083333333333326,
            0.08072916666666664, 0.028645833333333325, 0.0039062499999999987
        ]
        actual = list(glu)
    ...
    def test_fitting_flux(self):
        ...
        # call nlopt
        state, RSS_bestfit, flux_opt_slsqp = self.model.fitting_flux(
            method='LN_PRAXIS', flux=flux_initial1)
        actual = state["text"]
    ...
    def test_get_thres_confidence_interval(self):
        ...
        mdv1 = self.model.generate_mdv(flux_opt, cs)
        self.model.set_experiment('ex1', mdv1, cs)
        thres, number_of_measurements, degree_of_freedom = self.model.get_thres_confidence_interval(
            flux_opt, alpha=0.05)
